...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 5 {
        eprintln!("Usage: {} <input.r1cs> <out.ark-pkey> <out.ark-vk> <out.vk-json>", args[0]);
        std::process::exit(1);
    }
    ...
    let params = Groth16::<Bn254,LibsnarkReduction>::generate_random_parameters_with_reduction(circom, &mut thread_rng())?;
